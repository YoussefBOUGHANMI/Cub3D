...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/19 12:24:54 by yboughan          #+#    #+#             */
/*   Updated: 2023/03/19 16:23:33 by yboughan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calcul_h_dist(t_cub3d *data, t_rt *rt, float angle)
{
	...
	while (top == 0)
	{
		...
		else if ((data->py > rt->h_ry)
			&& data->map[((int)rt->h_ry >> 5) - 1][(int)rt->h_rx >> 5] == '1')
			top = 1;
	...
	rt->h_dist = dist(data, rt->h_rx, rt->h_ry, angle);
}
